    ...
    <div className={styles._card}>
      <div className={styles.card__mainInfo}>
        <div className={styles.card__mainInfo_img}>
          <img src={img} alt="" />
          <button onClick={() => { setFavorite(!favorite); return (favorite ? FavoritesStore.removeFavoriteId(id) : FavoritesStore.addFavoriteId(id)) }} className={favorite ? classnames(styles.favorite__btn, styles.favorite__btn_active) : styles.favorite__btn}>
        ...
        <div className={styles.card__mainInfo_text}>
          ...
          <p className={styles.card__text_description}> {description} </p>
        </div>
      </div>
      <div className={styles.card__priceInfo}>
        <div className={styles.card__priceBox}>
          <p className={styles._price}> {price} </p>
          <p className={styles._oldPrice}> {oldPrice} </p>
        </div>
        <div className={styles.card__priceBox_cartBtn}>
          <InvertBtn onClick={() => { setBasket(!basket); return (basket ? BasketStore.removeBasketId(id) : BasketStore.addBasketId(id)) }} style={basket ? { backgroundColor: "white", border: '0', borderRadius: '32px', content: 'В корзине' } : { backgroundColor: "black", border: '0', borderRadius: '32px', content: 'В корзину' }}  > {basket ? 'В корзине' : 'В корзину'} </InvertBtn>
        </div>
      </div>
    </div>
  )
